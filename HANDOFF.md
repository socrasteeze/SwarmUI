...
**Updated:** 2026-09-23 (18:36 PT, live Windows) · **Branch:** `main` · **Base:** `d1af6ac1` · **Tree:** clean
...
## Done this session
- 2026-09-23 (18:36 PT, live Windows box): routine automated sync — `upstream/master` at `96a4c3d1`, already an ancestor of HEAD (`d1af6ac1`); nothing to merge, no SwarmUI source gates. Fast-forwarded live backend node `comfyui-manager` `4ef99f88`→`9c354fb2` (3 commits, DB list only). Skipped dirty checkouts: DLNode ReActor (behind 17 + local `nodes.py`), TeaCache (`nodes.py`), WD14-Tagger / RMBG (`requirements.txt`). Left `.disabled` folders alone. Bundled ComfyUI current at `f427c3a28`. Logged in AGENTS.md Upstream Sync Log and pushed via `/clean`. Ran unattended. All earlier handoff detail above is unchanged.
- 2026-09-23 (12:40 PT, live Windows box): routine automated sync — `upstream/master` at `96a4c3d1`, already an ancestor of HEAD (`5b12ec7f`); nothing to merge, no SwarmUI source gates. Fast-forwarded bundled ComfyUI `b5cc88302`→`2255709aa` (4 commits; pip-installed `comfyui-workflow-templates` 0.11.68→0.11.69) and custom_node `RES4LYF` `056bc24`→`3d1d69d`. GPU accel check OK on both Main ComfyUI and SwarmUI backend. Skipped dirty checkouts: DLNode ReActor (behind 17 + local `nodes.py`), TeaCache (`nodes.py`), WD14-Tagger / RMBG (`requirements.txt`). Left `.disabled` folders alone. Logged in AGENTS.md Upstream Sync Log and pushed via `/clean`. Ran unattended. All earlier handoff detail above is unchanged.
