...
**Updated:** 2026-09-10 · **Branch:** main · **Base:** 8555aded (= origin/main) · **Tree:** clean

## State
Hub and spoke both up and the spoke backend is finally healthy: `G18-API` reads `running` with 14 features after
the F: model root was mirrored onto the spoke. No source changes this session — everything below is config and tracker.
...
1. **SWR.86 — open the Interrogate modal once.** Server is up now; still never done. WD14 only in the Method dropdown, tagger options render, a WD14 round trip returns tags. Also exercise Character Sheet's "Analyze pose" (same helper).
2. **SWR.59 — set `qwenEdit2511FP8_v10.safetensors` to `qwen-image-edit`** in the Models tab. Two minutes.
3. **Route one generation to the spoke** — now actually testable. Pick `G18-API` from the `/simple` Generate caret and confirm it completes; this is the unexercised half of the spoke profile.
4. **SWR.27 — device pass.** Chokepoint for SWR.1, SWR.2 and the acceptance of SWR.80–83.
5. Delete the leftovers on the spoke's `S:\SwarmUI`: `Data\Settings.fds.bak`, `.bak2`, `launch-spoke.bat.bak`. The live files are correct.
6. Cosmetic: the spoke logs a thumbnail-decode error for `F:\Models\diffusion_models\krea\krea2_raw_bf16.preview.jpeg`. Undecodable preview image; the model indexes fine. Same on the hub.
7. AnimaDex — after the cache drain, restart the container, verify favourites search returns total 3, commit the staged change in that checkout.
