...
3. **Florence-2 was removed on 2026-09-11 — confirm the Interrogate modal still renders.** Build, tests and format are green, but the server was down when this landed, so the modal has never been opened without the caption block. Check: the Method dropdown lists WD14 only, the tagger options still show, and a WD14 round trip still returns tags. Character Sheet's "Analyze pose" uses the same helper and should be checked in the same pass.
