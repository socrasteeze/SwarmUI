...
- `src/BuiltinExtensions/MobileEnhancements/**` — mobile/PWA extension (Phases 1–4 implemented: PWA manifest + service worker, touch image viewer with swipe navigation, mobile layout/ergonomics CSS, network resilience; plus a Civitai share-to-download target: PWA `share_target` → `/ShareTarget` route → `mobile_share.js` prefills the Model Downloader). Zero core-file edits; behavioral couplings tracked in the plan doc's coupling watchlist. **Not yet browser-verified** — build/format/boot could not run in the authoring environment (egress policy blocked the .NET SDK download); validated by static analysis + JS/JSON syntax checks. Run the verification gate before relying on it.
