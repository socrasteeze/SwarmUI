...
- **`/simple` prompting-ergonomics pass** (2026-08). Three moves, all inside the extension, all driven by what the on-screen keyboard covers.
  ...
  - **Autocomplete strip moved below the prompt box, and pins to the top of the keyboard.** The strip now lives in an `.m-ac-slot` wrapper inserted after the box. The wrapper is what holds the 44px of flow space open; the strip itself is what moves. Three things are load-bearing:
    ...
    **Enter/Tab accept, Escape dismisses** (`MAutoComplete.onKeyDown`), matching the genpage popover. The first chip carries `.m-ac-sel` so the key is not a guess — border only, never a fill, since "return will apply this" must not look like "already applied".
    A key is intercepted **only while chips are actually on screen**. This is a prompt textarea and a newline is a legitimate thing to want, so an empty (or Escape-dismissed) strip hands Enter straight back. Accepting routes through the chip's own `click()` rather than reimplementing the splice: the chip already knows its anchor, and the user's configured suffix is baked into the entry server-side by `AutoCompleteListHelper.GetData`, so a second path here would be a second place for both to drift.
    ...
    Browser-verified against a live server (Chromium): `hatsu` + Enter yields `hatsuko, ` with the suffix; Enter with no chips is not intercepted; Escape empties the strip and frees Enter; Tab accepts identically.
...
- `launch-windows.bat` — **core-file edit, one block, low risk**. The exit-42 restart branch now re-enters `launch-fork.bat` when it exists, falling back to `launch-windows.bat` otherwise, and calls by explicit `%~dp0` path instead of bare name. Keeps an in-app restart inside the fork wrapper. The bare-name form also fails outright with "not recognized" when `NoDefaultCurrentDirectoryInExePath` is set.
