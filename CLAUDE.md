...
- `src/BuiltinExtensions/TagDex/**` — the character/artist tag picker, for booru-trained anime models (Anima, IllustriousXL, NoobAI). **Zero core-file edits.** Full detail in `docs/TagDex-Plan.md`; the essentials:
  ...
  **Not browser-verified.** Build, format, headless boot, the real 245k-row parse, every API route over HTTP, sharded thumbnail serving, and all asset/tab routes on a running server all pass — but nobody has clicked a card yet, and thumbnail generation has never run against a live backend. The `/simple` browse *sheet* and a batch thumbnail sweep are unbuilt; see the plan doc's "Remaining work".
- `src/BuiltinExtensions/MobileEnhancements/Assets/m/index.html` — **fork-owned edit, 2 script tags**. Loads TagDex's `tagdex_core.js` and `m_tagdex.js` into `/simple`. Needed because `/simple` builds its own script list instead of using `WebServer.PageFooterExtra`, so another extension has no injection hook there. Must stay after `m_autocomplete.js` (which `m_tagdex.js` wraps) and before `m_create.js`. No upstream counterpart, so no merge risk.
