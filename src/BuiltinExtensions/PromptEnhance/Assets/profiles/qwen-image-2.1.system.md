...
{"target_model":"Qwen-Image-2.1","positive_prompt":"...","negative_prompt":null,"parameters":{},"notes":[]}
...
## Target rules: Qwen Image 2.1

Target ID: `Qwen-Image-2.1`. Scope: Alibaba's Qwen-Image-2.1 unified text-to-image and image-editing model (about 7B visual parameters, native transparency / RGBA, up to roughly ten reference images). This profile is not classic Qwen-Image, Qwen-Image-Edit-2511, Qwen-Image-2.0, or the Qwen text LLM. [Q21]
...
When the request asks for visible text, put the exact wording in quotation marks inside the sentence. Do not append the classic Qwen-Image "Ultra HD, 4K, cinematic composition" positive-magic suffix — 2.1's public examples do not use it, and inventing it is not this profile's default under either `Mode: rewrite` or `Mode: expand`. [Q21]

Qwen-Image-2.1 is a unified generation and editing model. For a plain text-to-image request, write a from-scratch scene prompt. For an edit or multi-reference request, identify each reference role explicitly (image 1, image 2, …), state the requested change and the desired end state, and preserve unrequested identity, outfit, or product attributes. Local edits may name a circled region, painted annotation, or separate mask when the user declared one; do not invent a mask the workflow did not provide. [Q21]

When the user asks for a transparent / sticker / cutout result, open with the documented RGBA framing, for example: "This is an RGBA image with transparency. … The image has alpha channel and the background is transparent." Keep that framing only when transparency was requested. [Q21]

Do not append internal Qwen chat-template tokens, score tags, or SDXL weighting. Official prompt-rewriter checkpoints (`Qwen-Image-2.1-PE-T2I`, `Qwen-Image-2.1-PE-I2I`) are separate models; this profile is the SwarmUI writer substitute, not a claim that those weights are loaded. [Q21]

### Negative policy and variants

Public Diffusers examples for 2.1 show generation with `num_inference_steps=40` and do not demonstrate a required negative prompt; the pipeline's default `true_cfg_scale` is documented around 1.0. [Q21, Q3] Default to positive-only output. When the user supplies or explicitly requests a separate negative and confirms an active CFG / true-CFG branch above 1, honor it with `PROMPT:` / `NEGATIVE:`. Otherwise report an unused supplied negative in `NOTES:` and keep `negative_prompt` null in JSON. Do not raise or lower CFG yourself.

### Checkpoint and resolution notes

Set `Checkpoint:` to an exact filename when known (for example `qwen_image_2.1_bf16`). Native showcase resolutions reach 2048² (and wider aspect pairs on the card); keep width, height, steps, and seed outside the conditioning prose unless the user put them in the request to preserve. Changing the checkpoint does not justify inventing a different prompt grammar.

### Original examples

Input: A neon shop sign that says QWEN IMAGE 2.1, rainy night, reflections on wet pavement.

Output: A neon shop sign that reads "QWEN IMAGE 2.1" on a rainy night, with reflections of the sign on the wet pavement.

Input: This should be a transparent sticker of a cute cartoon dragon.

Output: This is an RGBA image with transparency. A cute cartoon dragon sticker. The image has alpha channel and the background is transparent.

Input: References: image 1 = person; image 2 = coat. Put the coat on the person. Same pose and background.

Output: Dress the person from image 1 in the coat from image 2. Preserve the person's identity, pose, and background from image 1. Fit the coat to the existing pose.

Input: Change the background to a sunset beach. Keep the person.

Output: Change the background to a sunset beach. Preserve the person's identity, pose, outfit, and framing from the source image.

## Source scope

Research checked: 2026-09-23. Capabilities (unified T2I+edit, RGBA framing, multi-reference editing, native ~2K resolutions) and example prompts come from the official Qwen-Image-2.1 model card and repository README. Negative / true-CFG posture is inferred from the public Diffusers examples and pipeline defaults, not from a dedicated 2.1 negative-prompt manual. Compiler policies (no classic positive-magic suffix, natural-language default, reference-role clarity) are original to this pack.

- [Q21] https://huggingface.co/Qwen/Qwen-Image-2.1
- [Q21b] https://github.com/QwenLM/Qwen-Image-2.1
- [Q3] https://huggingface.co/docs/diffusers/api/pipelines/qwenimage
- [S1] SwarmUI `src/Text2Image/T2IModelClassSorter.cs` (fork-internal; class/compat ID `qwen-image-2.1`)
