...
class GenTabLayout {
    ...
    init() {
        ...
        document.addEventListener('touchstart', (e) => {
            ...
            // Also exclude contenteditable elements, not just INPUT/TEXTAREA/SELECT by tag - a touch-start
            // inside any editable content must never be misread as a panel-swipe gesture and hijack text
            // selection (the original bug this guarded against, on the prompt textarea).
            if (e.touches.length == 1 && !['BUTTON', 'INPUT', 'SELECT', 'TEXTAREA'].includes(e.target.tagName) && !e.target.isContentEditable && !findParentOfClass(e.target, 'model-block')) {
        ...
        document.addEventListener('touchend', (e) => {
            ...
            // Horizontal swipe toggles the left/right sidebars; vertical swipe toggles the bottom tool bar
            // (eg LoRAs). Horizontal swipe was previously removed here because it doubled as a false trigger
            // while selecting/highlighting prompt text - but that was a touchstart-filter gap (TEXTAREA/
            // contenteditable weren't excluded), not a reason to drop the gesture. That filter is fixed above
            // (touchstart never sets swipeStartX/Y for a touch starting in editable content), so a touch
            // reaching here by definition didn't start on editable text, and both axes are safe to handle.
        ...
        if (window.visualViewport) {
            // Routed through scheduleReapply, not reapplyPositions directly, for the same reason as the window
            // resize listener above: iOS fires a stream of visualViewport resize/scroll events while the
            // on-screen keyboard animates, and coalescing them to one rAF-scheduled layout pass avoids a
            // synchronous full relayout per event.
            let vvHandler = () => {
