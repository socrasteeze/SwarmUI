...
| `Assets/m/m_autocomplete.js` | Ported from `prompttools.js` (match/sort buckets, `<tag:` prefix set, the splice-at-index apply) and the parse loop in `main.js loadUserData`; `GetMyUserData.autocompletions` line format (`name\nlow\ntag\ncount\nalts`); `GetUserSettings` returning `autocomplete.matchmode`/`sortmode`; `util.js` `getTextContent`/`setTextContent`/`setTextSelRange`/`largeCountStringify`; `site.css` `.tag-type-0..9` | a changed line format shows as garbled chips; new prompt-syntax tags upstream will not appear here until the prefix table is updated (it is a copy, not a read) |
