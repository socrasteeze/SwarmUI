...
3. **Sidebar swipe animation polish** — **PREMISE STALE, re-scope before working this.** It was written to fix a `layout.js:465` TODO that upstream has since removed, so there is no longer a stated upstream complaint to fix; whether the animation is still wanted is an open question, and the design below assumes a `reapplyPositions()` shape that may also have moved. Original design, kept for reference (fixes it without editing layout.js): `mobile_core.js` adds a document-level `touchend` listener that briefly (~250 ms) adds an `.anim-bars` class to `body` so `reapplyPositions()`'s style writes animate via CSS transitions, then removes it — desktop splitter drags never animate. Fall back to opacity/transform-only open/close animation if the positioning styles don't transition cleanly. Keep small and reversible.
