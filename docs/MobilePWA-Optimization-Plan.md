...
- **Native app shell + mobile perf** — in progress (device-testing). `mobile_shell.js` (+ shell CSS in `mobile.css`) adds a fixed bottom tab bar (Create/History/Models/More) and a slide-up parameters drawer, driving the core Generate DOM via `.click()` / `genTabLayout`, scoped to `body.small-window.mobile-shell-active`, opt-out via the More sheet. Reserves its space by growing `body` padding-bottom (no core layout edit). Perf: `layout.js` gained `scheduleReapply()` (rAF-coalesced) on the prompt-input/resize handlers, and the `eval()` advance-width pass is now skipped on `small-window` (dead there) — kills the per-keystroke relayout jank. **Not browser-verified** (no SDK in authoring env); needs on-device iPhone testing per the gate.
...
| `mobile_shell.js` | Global `genTabLayout` (`setBottomShut`, `reapplyPositions`); top-tab button ids (`text2imagetabbutton`, `utilitiestabbutton`, `usersettingstabbutton`, `servertabbutton`, `simpletabbutton`); bottom sub-tab ids (`imagehistorytabclickable`, `modelstabheader`); `.input-sidebar` (`#input_sidebar`) drawer reuse; `sessionReadyCallbacks` | those ids/globals still exist; `reapplyPositions()` still subtracts `body` padding-bottom from heights (the shell relies on that to reserve the bottom-nav space with no core edit). **Also (found 2026-08):** `mobile.css`'s drawer rule targets the shared `#input_sidebar` by class (`.input-sidebar`), while `genpage.css` targets it by ID (`.small-window #input_sidebar { z-index: 25; ... }`) — an ID always outranks any number of classes on specificity, so the drawer's own `z-index` must stay `!important` or it silently loses to 25. Separately, the shell never syncs `genTabLayout.bottomShut`, so leaving `bottomShut == false` (e.g. from the shell's own History/Models nav) keeps core's `.mobile-layout-scrim` on at z-index 31 with `pointer-events: auto` while the drawer is open — re-check both z-index precedence and `bottomShut` state after any upstream change to either rule or to `getMobileGestureForTouch`/`syncMobilePanelClasses` |
...
| `mobile_core.js` `kb-animating` ↔ `mobile.css` transition scoping ↔ `AdvancedPopover.reposition()` | The keyboard-lift trio: the `--kb-inset` value + `> 120` `kb-open` threshold, the `kb-animating` edge class, and the popover's visual-viewport band math | re-check all three together if any one changes; the transition must stay off the element's resting state or the prompt bar trails the viewport during scroll again |
