...
Re-verified 2026-09-01 after upstream merge `b24d62b3` (site.js, prompttools.js, T2IPromptHandling.cs, LegacyPromptParser.cs). Every row below was checked against `git diff 3ad35ec0 b24d62b3` (actually isolated with `git diff f5c81adf b24d62b3`, since `3ad35ec0` is the merge-base of both parents and includes fork-only commits too) plus a live grep of the current tree. Nothing was broken; all fixes below are anchor-only. `browsers.js`, `permissions.js`, `WebServer.cs`, `Utilities.cs`, `T2IEngine.cs`, and `m_ui.js`/`m_app.js` were **not** touched by this merge at all — only `site.js` (~70 lines) and `prompttools.js` (+3 lines) were.

| Anchor | Risk if it changes |
|---|---|
| `prompttools.js` `getPossibleList` (~208) | The wrap target. Fails loud — `install()` probes for the method and bails with a console note. — Note: merge added a 3-line `weight[1.5]` prefix registration above this (line ~19), shifting everything below by +3; the method itself is untouched and `install()`'s wrap still resolves it. |
| `prompttools.js` `onInput` entry contract (~359-411, `'tag' in val` now at 369) | `raw`/`name`/`clean_html`/`tag`/`count_display`, and `'tag' in val` as the splice-anchor switch. Could fail **silently**. Highest-priority check. — Note: contract fields unchanged, only shifted +3 by the same insertion above. |
| The `<prefix:` gate we duplicate (`prompttools.js` ~277-284) | Four replicated lines (mirrored, not literal, in `tagdex_core.js` `augment()` ~366-370). Wrong behavior inside `<lora:` contexts if syntax detection changes. Fails soft. — Note: shifted +3 only; logic identical. |
| `browsers.js` `GenPageBrowserClass` ctor (:101), `describe` contract (consumed in `buildContentList` ~427-594: `desc.searchable`/`className`/`buttons`/`description`/`image`/`dragimage`, plus `file.data.src`) | Unconditional `desc.buttons.filter` and `file.data.src`. Breakage is loud. This file is already fork-modified. — Note: `tagdex_tab.js`'s `describe()` sets `record.src = record.thumb \|\| '';` before wrapping as `{name, data: record}`, satisfying `file.data.src`; confirmed still present. |
| `browsers.js` `refillTree` `/` splitting (~312-357) | The copyright-slug hazard (fact 3). |
| `browsers.js` `update()` folder-only cache (~250-277) | Every search/facet change must use `lightRefresh()`, never `update()`, or results go stale within a folder. — Note: `tagdex_tab.js.requery()` still calls `lightRefresh()` only. |
| `browsers.js` `refresh()` (~225-236) and first-build listener registration (`if (!this.hasGenerated)` block ~676-845, document pointer listeners at ~841-843) | After a download of the *active* dataset, `tagdex_tab.js` calls `refresh()` — `lightRefresh()` would leave a stale folder tree for anyone not at root. The `if (!this.hasGenerated)` block is also why the browser is never reconstructed: its document listeners are never removed. — Note: this anchor had drifted furthest (was ~790-814); now confirmed at ~676-845. |
| `browsers.js` `browserUtil.makeVisible` lazy-reveal contract (`helpers/browsers.js` ~29-45) | The batch sweep's `collectPlaceholderCards` reads `img.getAttribute('src') \|\| img.dataset.src` because an unrevealed card has only the latter, and `repaintCardThumb` mirrors the reveal (`classList.remove('lazyload')` + `delete dataset.src`) so a later reveal pass cannot overwrite a freshly generated thumb back to the placeholder. If the `lazyload` class or `dataset.src` handoff changes, generated references silently revert on scroll and the next sweep regenerates them. |
| `permissions.js` `hasPermission` fail-open before load (:53-56), `gather()` at `setTimeout(0)` (:7-9) | The dataset UI gates on `tagdex_manage`. Genpage is safe (session-ready callback); `/simple` must check at sheet-open, never at row build. — Note: byte-identical line numbers to last check; `m_tagdex.js` (~456-465) still checks at sheet-open. |
| `m_ui.js` `registerMoreItem` (:120) / `m_app.js` `buildMore` (:60, registered :29) | Fork-owned both sides. If the More tab stops appending `mUI.moreItems`, the `/simple` dataset sheet silently disappears. — Note: `m_ui.js` gained lines from the fork's own `f5c81adf` (Characters tab / More-in-header) commit, not from this merge; `m_tagdex.js` still feature-detects `mUI.registerMoreItem` before calling it. |
| `WebServer.cs:369-397` tab discovery (`Tabs/Text2Image/*.html`) and its `Tabs/GenerateBottom/*.html` sibling block just below it | Tab silently disappears. — Note: TagDex's `Tabs/GenerateBottom/Characters.html` still present. |
| `WebServer.cs:352-353` `ScriptFiles` → `ExtensionSharedFiles` | `/simple` stops loading `tagdex_core.js`. |
| `m_autocomplete.js` `getPossibleList` (~348), `buildChip` (~603) | Fork-owned, so only our own changes can break it. — Note: large drift (was ~233/~386) from the fork's own recent mobile commits, not this merge. |
| `Utilities.DownloadFile` (`Utilities.cs:906`) | Download progress signature. — Note: drifted from `:707`; `TagDexAPI.cs:276` call site still passes only `(url, filepath, progressLambda)`, unaffected since the `session` param stays optional/trailing. |
| `T2IEngine.CreateImageTask` (`T2IEngine.cs:181`, 7-arg overload — `:187` is a separate 9-arg overload) | Was "only matters once thumbnail generation exists" — that is now true: `TagDexThumbs.cs:89` calls this exact overload for reference-image generation, argument order still `(user_input, batchId, claim, output, setError, isWS, saveImages)`. GridGenerator/CharacterSheet call the same method, so upstream must fix its own caller first. |
