...
- **Phase 2 (browse tab)** — done. `Tabs/Text2Image/Characters.html` auto-wires a Characters tab; `GenPageBrowserClass` in Cards format with a copyright folder tree, five facet dropdowns, server-driven search, and per-character core-tag chips.
...
8. **Extension tabs auto-wire with zero core edits** — `WebServer.cs:345-359` discovers `Tabs/Text2Image/*.html`, registers a `view_extension_tab_{id}` permission, and injects nav item plus pane. `Characters.html` → pane id `characters`, permission `view_extension_tab_characters`.
...
src/BuiltinExtensions/TagDex/
  ...
  Tabs/Text2Image/Characters.html
```
